  ...
  describe '#calculate' do
    ...
    context 'when the input is a string with an arbitrary number of numbers' do
      it 'returns the sum' do
        ...
        result = calculator.calculate(string)
    ...
    context 'when the input is a string with a single negative number' do
      it 'raises an exception' do
        expect{calculator.calculate("-1")}.to raise_error(described_class::NegativeNumberError, "negative numbers are not allowed -1")
      end
    end

    context 'when the input is a string with multiple negative numbers' do
      it 'raises an exception' do
        expect{calculator.calculate("-1,-2,-3")}.to raise_error(described_class::NegativeNumberError, "negative numbers are not allowed -1, -2, -3")
      end
    end

    context 'when input is a string with special conditions' do
      it 'returns the sum excluding numbers greater than 1000' do
        result = calculator.calculate('2,3,1001')
    ...
    context 'when the second input is a *' do
